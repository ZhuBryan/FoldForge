...
ANIMAL_SPECS = [
    ("butterfly", "Blue morpho butterfly", None, "off", "butterfly"),
    ("butterfly_sym", "Butterfly (symmetrized)", None, "y", "butterfly"),
    ("swallowtail", "Swallowtail butterfly", None, "auto", "swallowtail"),
    ("moth", "Giant peacock moth", None, "auto", "moth"),
    ("ginkgo", "Ginkgo leaf", None, "auto", "ginkgo"),
    ("starfish", "Starfish", (120, 25, 835, 700), "off", "starfish"),
    ("fish", "Goldfish (side profile)", None, "off", "fish"),
]
...
<div id="ui">
 ...
 <button id="foldBtn" title="Open a FOLD crease-pattern file (.fold) and animate its fold.">&#128193; Open .fold</button>
 <input id="foldfile" type="file" accept=".fold,application/json" style="display:none">
 <button id="dl" title="Download the current folded frame as a Wavefront .obj mesh.">&#11015; Export OBJ</button>
 <!-- panel / appearance toggles (green = on) -->
 <button id="texBtn" title="Texture the folded sheet with the source photo, or show blank paper. The reference photo always stays in its own panel.">&#128444;&#65039; Texture: Paper</button>
 <button id="heatBtn" class="on" title="Show or hide the photo pipeline strip (input &rarr; height field &rarr; symmetrized). Green = shown.">&#128200; Pipeline</button>
 <button id="galleryBtn" title="Show or hide the showcase gallery of baked sample results. Green = shown.">&#127912; Gallery</button>
 <!-- playback -->
 <button id="play" title="Play or pause the folding animation.">&#10073;&#10073; Pause fold</button>
...
function computeChain(t,step){
  ...
  if(ch.kind==='cols'){
    ...
    const spx=new Float64Array(np1),spz=new Float64Array(np1); let h=0,x=ch.sx,z=t*ch.sz; spx[0]=x; spz[0]=z;  // scale the row-anchor by the fold fraction so fold=0 is a flat sheet
...
detail.addEventListener('change',()=>{detailNx=parseInt(detail.value)||24;
  const cur=DATA.shapes[sel.value];
  if(sel.value==='__up'&&lastImg) rebuildImage();                 // uploaded photo: full re-segment/fold
  else if(cur&&cur.mode==='subject'&&cur.seg) refoldSubject(sel.value); // baked/loaded sample: re-fold its height field
  else if(lastImg) rebuildImage();
  else setStatus('Detail sets the crease budget for folded photos and samples — load a sample or upload a photo; “'+((cur&&cur.label)||sel.value)+'” is a fixed pattern.',''); });
...
function syncTexBtn(){ texBtn.innerHTML='&#128444;&#65039; Texture: '+(photoTex?'Photo':'Paper'); }
texBtn.addEventListener('click',()=>{photoTex=!photoTex; texBtn.classList.toggle('on',photoTex); syncTexBtn();
...
function rebuildImage(){ if(!lastImg)return;
  ...
  sel.value='__up'; buildShape('__up'); t=0;dir=1;playing=true;play.innerHTML='&#10073;&#10073; Pause fold';
  ...
  else setStatus('Folded '+nm+' — '+info+', '+iw+'×'+ih+', '+presetLabel()+' — '+folds+' folds'+smsg,'ok');
}
// Detail change on a baked/loaded subject sample (no live upload behind it): re-fold
// its stored height field (seg blob) at the new crease budget, through the very same
// buildImageShape/buildMiuraShape path uploads use — so Easy/Detailed visibly change
// the mesh geometry and the fold-count readout. The baked photo texture is reused.
function refoldSubject(name){
  const cur=DATA.shapes[name]; if(!cur||!cur.seg) return;
  const seg=ensureSeg(cur.seg), nx=detailNx;
  const bw=seg.bbox[2]-seg.bbox[0]+1, bh=seg.bbox[3]-seg.bbox[1]+1;
  const ny=Math.max(6,Math.round(nx*bh/Math.max(1,bw)));
  const shape=(engineMode==='miura')?buildMiuraShape(null,nx,ny,'subject',seg):buildImageShape(null,nx,ny,'subject',seg);
  shape.tex=cur.tex; shape._tex=null;                        // reuse the baked photo texture (realised lazily)
  shape.seg=seg; shape.segRaw=cur.segRaw||null; shape.sym=cur.sym||null;
  shape.engine=cur.engine||shape.engine; shape.label=cur.label||shape.label;
  DATA.shapes[name]=shape;                                    // replace in place, same dropdown/gallery key
  buildShape(name); t=0;dir=1;playing=true;play.innerHTML='&#10073;&#10073; Pause fold';
  const folds=nx-1; let smsg='';
  if(shape.sym&&shape.sym.iou!=null) smsg=' · mirror-IoU '+shape.sym.iou.toFixed(2)+(shape.sym.applied?' (symmetrized)':'');
  if(shape.fitInfo){const fi=shape.fitInfo;
    setStatus('Folded '+shape.label+' — live 2-D Miura fit on a '+fi.R+'×'+fi.C+' grid, fidelity error '+fi.error.toFixed(3)+', '+presetLabel()+' — '+folds+' folds'+smsg,'ok');}
  else setStatus('Folded '+shape.label+' — '+presetLabel()+' — '+folds+' folds'+smsg,'ok');
}
...
function loadBaked(n){ sel.value=n; buildShape(n); t=0;dir=1;playing=true;play.innerHTML='&#10073;&#10073; Pause fold'; }
...
buildGallery();
sel.addEventListener('change',()=>{buildShape(sel.value);t=0;dir=1;playing=true;play.innerHTML='&#10073;&#10073; Pause fold';});
...
// The readout names the *selected* preset (Easy/Medium/Detailed/Extra) rather
// than re-deriving a difficulty band from the raw fold count — the two used to
// disagree (Detailed's ~23 folds fell in the "Easy" band, reading "23 folds
// (Easy)"). Pull the preset name straight from the chosen dropdown option.
function presetLabel(){ const o=detail.options[detail.selectedIndex]; return o?o.text.split(' ')[0]:''; }
...
function foldImageFile(file){
  ...
  // Keep the object URL alive: the pipeline strip's "input photo" panel loads
  // the same URL into its own <img>, so revoking it on load (as we used to) left
  // that panel with a dead blob URL and a broken/empty box. Free the *previous*
  // upload's URL instead, so at most one is ever held.
  if(lastImgUrl) URL.revokeObjectURL(lastImgUrl);
  const url=URL.createObjectURL(file), img=new Image(); lastImgUrl=url;
  img.onload=()=>{
    try{ lastImg=img; img._name=file.name; rebuildImage(); }
    catch(err){ setStatus('Could not fold '+file.name+': '+err.message,'err'); }
  };
  img.onerror=()=>{ if(lastImgUrl===url){URL.revokeObjectURL(url); lastImgUrl=null;}
...
function foldFoldFile(file){
  ...
  const rd=new FileReader(); rd.onload=()=>{ try{const fold=JSON.parse(rd.result); const shape=buildFoldShape(fold);
    ...
    sel.value='__fold'; buildShape('__fold'); t=0;dir=1;playing=false;play.innerHTML='&#9654; Play fold'; slider.value=0;
